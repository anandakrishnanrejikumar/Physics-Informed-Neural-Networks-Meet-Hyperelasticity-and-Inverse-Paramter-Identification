...
def Extract_nodes_force_for_loss(connectivity, nodes, dof):
...
def Calculate_totalLoss_batch(total_energy, force):
    alpha = config["Hyperparameters"]["Alpha"]  # forward problem
    # Compute the norm of the forces for each element in the batch
    force_norms = (torch.linalg.norm(force, dim=1)**2)  # Shape: (N, )
    force_norms = torch.sum(force_norms)
    # Compute L_R for each batch element
    T_R = total_energy + alpha*force_norms  # Shape: (N, )
    return T_R, force_norms
...
def main(nEl, connectivity, displacement_all, preCalc):
    # Get quadrature points and weights
    quadrature_point, weights = quadrature_points()
    ...
    # displacement = displacement.clone().detach().requires_grad_(True)
    # Delete
    ...
    F = torch.eye(ed.dof_per_node, dtype=torch.float64).unsqueeze(0).unsqueeze(0).unsqueeze(0).repeat(nLoadSteps, nEl, 8,
                                                                                                     1,
                                                                                                     1)  # (10, 8, 3, 3)
    F += torch.einsum('eqijk, bei -> beqjk', shape_function_gradients,
                      displacements_all_time_steps)  # (10, 64, 8, 3, 3)

    # All determinants in one go
    detF = torch.linalg.det(F)  # (10, 64, 8)

    # C = torch.einsum('bqij, bqjk -> bqik', F.transpose(2, 3), F) # Shape (10, 8, 3, 3)
    F_reshaped = F.view(-1, 3, 3)  # Shape (10*8, 3, 3)
    C_reshaped = torch.bmm(F_reshaped.transpose(1, 2), F_reshaped)  # Shape (10*8*64, 3, 3)
    C = C_reshaped.view(nLoadSteps, nEl, 8, 3, 3)  # Shape (10, 64, 8, 3, 3)

    # Compute energy and stress across all load steps and qps
    E = Compute_energyNeoHooke_batch(C, detF)  # Shape (10, 64, 8)
    # S = Compute_secondPiolaStress_batch(C, detF)  # Shape (10, 64, 8, 3, 3)

    # Assemble right hand side vectors
    # cellRHS = Calculate_internalForceVector_batch(nEl, S, F, shape_function_gradients, weights,
    #                                               detJ2)  # Shape (10, 64, 24)

    # Compute energy (Shape (10,))
    energy_function = (weights * detJ2.view(1, nEl, 8) * E).sum(dim=2).sum(dim=1).sum(dim=0)
    ...
    # rightHandSide = torch.zeros(nLoadSteps, ed.dof_counter, dtype=torch.double)
    # dofMatrix_flatten = torch.from_numpy(dofMatrix).flatten()
    # for i in range(nLoadSteps):
    #     rightHandSide[i].put_(dofMatrix_flatten, cellRHS[i].flatten(), accumulate=True)
    #
    # # Set rightHandSide at constrained dofs to zero across all loadsteps
    # rightHandSide_condensed = Extract_freeDofs_batch(rightHandSide)

    # #Automatic Differentiation
    rightHandSide = torch.autograd.grad(energy_function, displacement, create_graph=True)[0]
    rightHandSide_condensed = Extract_freeDofs_batch(rightHandSide.view(nLoadSteps,-1))

    # Calculate overall loss
    loss, residual_force_norm = Calculate_totalLoss_batch(energy_function, rightHandSide_condensed)
    return loss, residual_force_norm
